...
export const PedidosPage: React.FC = () => {
  ...
  const handleAbrirPagamento = (pedido: Pedido) => {
    // Sincronizar carrinho com as reservas atuais antes de ir
    const reservas = pedidos.filter(p => p.status === 'RESERVED');
    const novosItens = reservas.map(p => ({
      idPedido: p.id,
      produto: produtos.find(prod => prod.id === p.itens[0].produtoId)!,
      quantidade: p.itens[0].quantidade
    })).filter(item => item.produto !== undefined);

    setCarrinho(novosItens);
    navigate('/marketplace', { state: { openCart: true } });
  ...
  const gerarComprovante = (pedido: Pedido) => {
    ...
    doc.text('NextPay - Comprovante', 20, 20);
              ...
              <div className="space-y-6">
                <div className="grid gap-3">
                  <button
                    ...
                    className={`p-6 rounded-3xl border-2 transition-all flex items-center gap-5 ${metodoPagamento === 'pix' ? 'border-petroleo-600 bg-petroleo-50 shadow-inner' : 'border-gray-50 hover:border-gray-100'
                      }`}
                  >
                    <div className="w-14 h-14 bg-petroleo-100 rounded-2xl flex items-center justify-center shrink-0">
                      <TrendingUp className="text-petroleo-600" size={28} />
                    </div>
                  ...
                  <button
                    ...
                    className={`p-6 rounded-3xl border-2 transition-all flex items-center gap-5 ${metodoPagamento === 'transferencia' ? 'border-petroleo-600 bg-petroleo-50 shadow-inner' : 'border-gray-50 hover:border-gray-100'
                      }`}
                  >
                    <div className="w-14 h-14 bg-petroleo-100 rounded-2xl flex items-center justify-center shrink-0">
                ...
                <div className="bg-gray-900 p-8 rounded-[32px] text-white">
                  ...
                  <div className="flex justify-between items-end">
                    <p className="text-4xl font-black text-petroleo-300">
                    ...
                    <div className="flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/10">
                      <ShieldCheck size={12} className="text-petroleo-300" />
                      Protegido
                    </div>
                  </div>
                </div>
