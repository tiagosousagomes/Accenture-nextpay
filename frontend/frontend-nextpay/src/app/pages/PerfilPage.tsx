...
export const PerfilPage: React.FC = () => {
  ...
  const handleSalvar = async () => {
    try {
      ...
      if (fotoBase64) {
        // Backend armazena base64 puro e adiciona o prefixo data URI no getter.
        // Enviar somente o conteúdo após a vírgula evita prefixo duplicado.
        dataToSend.fotoPerfil = fotoBase64.includes(',')
          ? fotoBase64.split(',')[1]
          : fotoBase64;
      }
      ...
      if (response.ok) {
        ...
        updateProfile({ ...formData, fotoPerfilUrl: updatedData.fotoPerfilUrl } as any);
      ...
      <div className="max-w-5xl mx-auto space-y-8 pb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-lg text-gray-600 mt-1">Gerencie sua identidade no ecossistema NextPay.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-yellow-50 px-4 py-2 rounded-full border border-yellow-200 text-yellow-700 font-bold text-sm">
              <Coins size={18} />
              <span>{apiUser?.pontuacao || 0} NextCoins</span>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-800">
                    {user?.createdAt ? new Date(user.createdAt).toLocaleDateString('pt-BR') : 'Maio 2024'}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
