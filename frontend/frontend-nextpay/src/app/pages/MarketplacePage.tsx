...
export const MarketplacePage: React.FC = () => {
  ...
  const { conta, refreshConta, saldoMoedas } = useBank();
  ...
  const [metodoPagamento, setMetodoPagamento] = useState<'pix' | 'transferencia' | 'moedas' | null>(null);
  const [isProcessando, setIsProcessando] = useState(false);
  const [moedasParaUsar, setMoedasParaUsar] = useState(0);
  const location = useLocation();

  useEffect(() => {
    if (location.state?.openCart) {
      setCarrinhoAberto(true);
      // Limpa o state para não abrir de novo se o usuário navegar de volta
  ...
  const handleFinalizarCompra = async () => {
    ...
    const valorFinal = totalCarrinho - (moedasParaUsar / 10);
    if (!conta || valorFinal > (conta.saldo || 0)) {
      toast.error('Saldo insuficiente na conta para o valor restante');
      return;
    }

    setIsProcessando(true);
    try {
      const sucesso = await confirmarPagamento(carrinho, moedasParaUsar);
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto space-y-10">
        {/* Hero Section */}
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-petroleo-900 via-petroleo-800 to-petroleo-950 p-12 text-white shadow-2xl">
          <div className="relative z-10 max-w-2xl">
            <h1 className="text-5xl font-extrabold tracking-tight mb-4">
              Explore o <span className="text-petroleo-300">Marketplace</span>
            </h1>
            <p className="text-petroleo-100 text-xl mb-8 leading-relaxed">
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
                <ShieldCheck size={18} className="text-petroleo-300" />
                <span className="text-sm font-medium">Compra Protegida</span>
              </div>
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
                <TrendingUp size={18} className="text-petroleo-300" />
          ...
          <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 w-96 h-96 bg-petroleo-400/20 rounded-full blur-[100px]"></div>
        ...
        {produtos.filter((p) => p.vendedorId === user?.id).length === 0 && (
          <div className="bg-gradient-to-r from-petroleo-900 to-petroleo-700 rounded-xl px-6 py-4 text-white shadow-md flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
            <div>
            ...
            <button
              ...
              className="px-6 py-2 bg-white text-petroleo-900 rounded-lg font-bold hover:bg-petroleo-50 transition shadow-md"
            >
              Começar a Vender
            </button>
          </div>
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <button
                  ...
                  className={`w-full group/btn flex items-center justify-center gap-2 py-4 rounded-2xl font-bold transition-all ${produto.estoque > 0 && lojaAberta
              ...
              {passoPagamento === 'carrinho' ? (
                carrinho.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Reservation Badge */}
                    <div className="bg-petroleo-50 border border-petroleo-100 p-4 rounded-2xl flex items-start gap-3">
                      <ShieldCheck className="text-petroleo-600 shrink-0" size={20} />
                      <p className="text-xs text-petroleo-900 font-medium leading-relaxed">
                ...
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-3">
                      <button
                        ...
                        className={`p-5 rounded-2xl border-2 transition-all flex items-center gap-4 ${metodoPagamento === 'pix'
                      ...
                      >
                        <div className="w-12 h-12 bg-petroleo-100 rounded-xl flex items-center justify-center">
                          <TrendingUp className="text-petroleo-600" size={24} />
                        </div>
                        <div className="flex-1 text-left">
                          <p className="font-bold text-gray-900">PIX Instantâneo</p>
                          <p className="text-xs text-gray-500">Saldo atual: R$ {conta?.saldo.toLocaleString('pt-BR')}</p>
                        </div>
                      </button>

                      {/* <button
                        onClick={() => setMetodoPagamento('transferencia')}
                        className={`p-5 rounded-2xl border-2 transition-all flex items-center gap-4 ${metodoPagamento === 'transferencia'
                          ? 'border-petroleo-600 bg-petroleo-50 shadow-sm'
                          : 'border-gray-100 hover:border-gray-200'
                          }`}
                      >
                      ...
                      {/* Moedas Cashback Section */}
                      <div className="mt-4 p-5 rounded-2xl border-2 border-dashed border-petroleo-200 bg-petroleo-50/30 space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-petroleo-100 rounded-lg flex items-center justify-center">
                              <Coins className="text-petroleo-600" size={20} />
                            </div>
                            <div>
                              <p className="font-bold text-sm text-gray-900">Usar Moedas Cashback</p>
                              <p className="text-[10px] text-petroleo-700 font-medium">Saldo: {saldoMoedas.toFixed(0)} moedas</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-xs font-black text-petroleo-600"><Coins className="text-petroleo-600" size={10} />{moedasParaUsar.toLocaleString('pt-BR')}</span>
                          </div>
                        </div>
                        ...
                        <input
                          type="range"
                          min="0"
                          max={Math.min(saldoMoedas, totalCarrinho * 10)}
                          value={moedasParaUsar}
                          onChange={(e) => setMoedasParaUsar(Number(e.target.value))}
                          className="w-full h-2 bg-petroleo-100 rounded-lg appearance-none cursor-pointer accent-petroleo-600"
                        />
                        <div className="flex justify-between text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                          <span>0</span>
                          <span>{Math.min(saldoMoedas, totalCarrinho * 10).toFixed(0)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-900 p-6 rounded-3xl text-white space-y-4">
                    ...
                    {moedasParaUsar > 0 && (
                      <div className="flex justify-between items-center text-petroleo-300 text-sm">
                        <span>Desconto Moedas</span>
                        <span>- R$ {(moedasParaUsar / 10).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center border-t border-white/10 pt-4">
                      <span className="font-bold">Total a pagar</span>
                      <span className="text-2xl font-black text-petroleo-300">R$ {(totalCarrinho - (moedasParaUsar / 10)).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                </div>
              ) : (
                ...
                {passoPagamento === 'carrinho' ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      className="w-full py-5 bg-petroleo-600 hover:bg-petroleo-700 text-white rounded-2xl font-black text-lg shadow-xl shadow-petroleo-600/20 flex items-center justify-center gap-3 transition-all disabled:opacity-50"
                    >
                      {isProcessando ? (
