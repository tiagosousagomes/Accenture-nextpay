...
export const ExtratoPage: React.FC = () => {
  ...
  const getTipoIcon = (tipo: Transacao['tipo']) => {
    const isEntrada = isEntradaTransacao(tipo);
    if (isEntrada) {
  ...
  const getValorColor = (tipo: Transacao['tipo']) => {
    const isEntrada = isEntradaTransacao(tipo);
    if (isEntrada) {
  ...
  const getSinal = (tipo: Transacao['tipo']) => {
    const isEntrada = isEntradaTransacao(tipo);
    if (isEntrada) {
      return '+';
    }
            ...
            {transacoesFiltradas.map((transacao, index) => (
              <div key={transacao.id} className="relative">
                ...
                <div className="flex gap-4 pb-8">
                  {/* Ícone */}
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${getTipoColor(transacao.tipo)} relative z-10`}>
                  ...
                  <div className="flex-1 bg-gray-50 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        <p className={`text-2xl font-bold ${getValorColor(transacao.tipo)}`}>
